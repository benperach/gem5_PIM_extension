...
class colReadVerifyLtEq : public ColumnOperation {
  private:
    void* addr2; // for debug
  public :
    colReadVerifyLtEq(void* _addr2) :
    addr2(_addr2) {};
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyLtEq* fcolReadVerifyLtEq =
            new colReadVerifyLtEq(addr2);
    ...
    printf("Sending mPIM ColLTE\n");
    // the src1 len is 7 because we write (len-1)
    pim_op    = pim::setOpTwoSrc(pim::COL_LTE,16,0,7,8,7);
    pimInst(pim_op,(uint64_t)addr2);

    printf("Start read and verify data\n");
    // go over all the columns and read the data
    colOp(addr2,resultCol,1,1,fcolReadVerifyLtEq);
    printf("Test end\n");
