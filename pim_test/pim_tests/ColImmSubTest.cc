...
class colReadVerifyImmSub : public ColumnOperation {
  private:
    uint8_t Imm;
    void* addr2; // for debug
  public :
    colReadVerifyImmSub(uint8_t _Imm, void* _addr2) :
    Imm(_Imm),addr2(_addr2) {};

    virtual void operator() (uint8_t* addr, int col, int subarray,
                                int row, int mat, int chip)
    {
        uint8_t result= *addr;
        if (result%256 != (uint8_t)(Imm-(uint8_t)row)%256) {
            printf("TestError: in row 0x%X, mat 0x%X, subarray 0x%X"
                    ", chip 0x%X, result = 0x%X, (Imm-row) = 0x%X\n",
                    row,mat,subarray,chip, result,
                    (uint8_t)(Imm-(uint8_t)row)%256);
        }/* else {
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyImmSub* fcolReadVerifyImmSub =
            new colReadVerifyImmSub(Imm,addr2);
    ...
    for (int i = 0; i < 256 ; i++){
        ...
        pim_op    = pim::setOpImm(pim::COLIMM_SUB,8,0,7,(uint8_t)i);
        pimInst(pim_op,(uint64_t)addr2);
        fcolReadVerifyImmSub->setImm((uint8_t)i);
        printf("Start read and verify data\n");
        // go over all the columns and read the data
        colOp(addr2,resultCol,1,1,fcolReadVerifyImmSub);
        printf("Start flushing data\n");
