...
class colReadVerifyEqScan : public ColumnOperation {
  ...
  public :
    colReadVerifyEqScan(void* _addr2) :
        comp_number(0),srcOffset(0),srcLen(0),addr2(_addr2)
         {};
    ...
    {
        uint8_t result= *addr;
        unsigned modMask = 1U << (srcLen+1);
        if ((row >> srcOffset) % modMask == comp_number % modMask)
        {
            if ((result & 1u) != 1) {
        ...
        } else if ((row >> srcOffset) % modMask != comp_number % modMask)
        {
            if ((result & 1u) != 0) {
    ...
    inline void setSrcLen(uint8_t newSrcLen){
        srcLen = newSrcLen;
    }
};
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    for (int i = 0; i < 256 ; i++){
        ...
        srcOffset = i % pim::BYTE_SIZE;
        srcLen = (pim::BYTE_SIZE - 1) - srcOffset;
        printf("Sending mPIM ScanEq(0x%X)\n",i);
        pim_op = pim::setOpImm(pim::SCAN_EQ,8,srcOffset,
                                    srcLen,(uint8_t)i);
        pimInst(pim_op,(uint64_t)addr2);
        fcolReadVerifyEqScan->setCompNumber((uint8_t)i);
        fcolReadVerifyEqScan->setSrcOffset(srcOffset);
        fcolReadVerifyEqScan->setSrcLen(srcLen);
