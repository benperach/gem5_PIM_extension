...
class colReadVerifyGtEqScan : public ColumnOperation {
  private:
    uint8_t comp_number;
    void* addr2; // for debug
  public :
    colReadVerifyGtEqScan(uint8_t _comp_number, void* _addr2) :
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyGtEqScan* fcolReadVerifyEqScan =
            new colReadVerifyGtEqScan(comp_number,addr2);
    ...
    for (int i = 0; i < 256 ; i++){
        ...
        pim_op    = pim::setOpImm(pim::SCAN_GTEQ,8,0,7,(uint8_t)i);
        pimInst(pim_op,(uint64_t)addr2);
