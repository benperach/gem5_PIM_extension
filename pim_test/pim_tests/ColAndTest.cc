...
class colReadVerifyAnd : public ColumnOperation {
  private:
    void* addr2; // for debug
  public :
    colReadVerifyAnd(void* _addr2) :
    addr2(_addr2) {};
    ...
    {
        uint8_t result = *addr;
        uint8_t reverse = reverse_bit((uint8_t)row)%256;
        uint8_t expected = (uint8_t)(row%256) & reverse;
        if (result%256 != expected) {
            printf("!!!TestError!!!!: in row 0x%X, mat 0x%X, subarray 0x%X"
                    ", chip 0x%X, result = 0x%X, reverse = 0x%X, "
                    "expected = 0x%X\n",
                    row,mat,subarray,chip,result,reverse, expected);
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyAnd* fcolReadVerifyAnd =
            new colReadVerifyAnd(addr2);
    ...
    printf("Sending mPIM ColAnd\n");
    // the src1 len is 7 because we write (len-1)
    pim_op    = pim::setOpTwoSrc(pim::COL_AND,16,0,7,8,7);
    pimInst(pim_op,(uint64_t)addr2);
    ...
    colOp(addr2,resultCol,1,1,fcolReadVerifyAnd);
    printf("Test end\n");
