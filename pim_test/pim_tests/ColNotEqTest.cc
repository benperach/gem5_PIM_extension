...
class colReadVerifyNotEq : public ColumnOperation {
  private:
    void* addr2; // for debug
  public :
    colReadVerifyNotEq(void* _addr2) :
    addr2(_addr2) {};
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyNotEq* fcolReadVerifyNotEq =
            new colReadVerifyNotEq(addr2);
    ...
    printf("Sending mPIM ColNEQ\n");
    // the src1 len is 7 because we write (len-1)
    pim_op    = pim::setOpTwoSrc(pim::COL_NEQ,16,0,7,8,7);
    pimInst(pim_op,(uint64_t)addr2);

    printf("Start read and verify data\n");
    // go over all the columns and read the data
    colOp(addr2,resultCol,1,1,fcolReadVerifyNotEq);
    printf("Test end\n");
