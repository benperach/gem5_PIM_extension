...
class colReadVerifyOr : public ColumnOperation {
  private:
    void* addr2; // for debug
  public :
    colReadVerifyOr(void* _addr2) :
    addr2(_addr2) {};
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyOr* fcolReadVerifyAnd =
            new colReadVerifyOr(addr2);
    ...
    printf("Sending mPIM ColOr\n");
    // the src1 len is 7 because we write (len-1)
    pim_op    = pim::setOpTwoSrc(pim::COL_OR,16,0,7,8,7);
    pimInst(pim_op,(uint64_t)addr2);
