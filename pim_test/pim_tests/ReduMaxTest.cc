...
class colReadVerifyReduMax : public ColumnOperation {
  private:
    unsigned comp_number;
    void* addr2; // for debug
    unsigned res_mask;
  public :
    colReadVerifyReduMax(void* _addr2):
                                ...
                                int startCol)
    {
        ...
        if (row == pim::MAT_DIM - 1){
            expected = comp_number & 0xFF;
        }

        uint8_t result = *addr;
        if ((result & res_mask & 0xFF) != expected)
        {
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyReduMax* fcolReadVerifyReduMax =
            new colReadVerifyReduMax(addr2);
    ...
    for (unsigned len = 0 ; len < pim::BYTE_SIZE ; len++){
        ...
        pim_op = pim::setOpImm(pim::REDU_MAX,resultCol*pim::BYTE_SIZE
                    ,0,len,0);
        pimInst(pim_op,(uint64_t)addr2);
        printf("Start read and verify data\n");
        // go over all the columns and read the data
        fcolReadVerifyReduMax->setInputLen(len+1);
        colOp(addr2,resultCol,resNumOfCol,1,fcolReadVerifyReduMax);
        // flush results
        // go over all the columns and flush the data
