...
class colReadVerifyImmAdd : public ColumnOperation {
  private:
    uint8_t Imm;
    void* addr2; // for debug
  public :
    colReadVerifyImmAdd(uint8_t _Imm, void* _addr2) :
...
int main(){
    ...
    *(uint8_t*)addr2 = 0x11;
    ...
    colReadVerifyImmAdd* fcolReadVerifyImmAdd =
            new colReadVerifyImmAdd(Imm,addr2);
    ...
    for (int i = 0; i < 256 ; i++){
        ...
        pim_op    = pim::setOpImm(pim::COLIMM_ADD,8,0,7,(uint8_t)i);
        pimInst(pim_op,(uint64_t)addr2);
        fcolReadVerifyImmAdd->setImm((uint8_t)i);
        printf("Start read and verify data\n");
        // go over all the columns and read the data
        colOp(addr2,resultCol,1,1,fcolReadVerifyImmAdd);
        printf("Start flushing data\n");
